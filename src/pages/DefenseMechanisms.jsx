...
import { ShieldCheck, BookOpen, PlayCircle, Headphones, AlertTriangle, CheckCircle2, TrafficCone, ArrowLeft } from 'lucide-react';
...
const markdownComponents = {
 h3: ({ node, ...props }) => <h3 className="text-xl font-semibold text-emerald-600 mt-4 mb-2" {...props} />,
 p: ({ node, ...props }) => <p className="text-sm text-[#444] leading-relaxed mb-2" {...props} />,
 ul: ({ node, ...props }) => <ul className="list-disc pr-5 mb-2 space-y-1 text-sm text-[#444]" {...props} />,
 ol: ({ node, ...props }) => <ol className="list-decimal pr-5 mb-2 space-y-1 text-sm text-[#444]" {...props} />,
 li: ({ node, ...props }) => <li {...props} />,
 blockquote: ({ node, ...props }) => (
 <blockquote className="border-l-4 border-emerald-600 pl-4 italic text-[#666] my-4" {...props} />
 ),
};
...
export default function DefenseMechanisms() {
 ...
 return (
 <div className="flex flex-col min-h-screen bg-bg-app space-y-6 px-5 pb-10 font-sans">
 {/* Header */}
 <header className="flex items-center pt-4 pb-2 justify-between">
 <button onClick={() => navigate(-1)} className="flex size-10 items-center text-text-primary">
 <ArrowLeft size={24} />
 </button>
 <h2 className="flex-1 text-center pr-10 text-lg font-bold text-text-primary">آليات الدفاع النفسي</h2>
 ...
 <h3 className="text-lg font-semibold text-text-primary">آليات الدفاع النفسي</h3>
 </div>
...
<div className="overflow-x-auto">
 ...
 className="space-y-6 p-6 bg-bg-surface rounded-xl shadow-md text-text-primary leading-relaxed"
 >
 <div className="flex items-center gap-3">
 <TrafficCone className="text-emerald-600" />
 ...
 {/* المثال */}
 {item.example && (
 <p className="text-sm italic text-emerald-600"><strong>مثال:</strong> {item.example}</p>
 )}

 {/* الاستخدام الصحي */}
 {item.healthy && (
 <p className="text-sm text-text-primary"><strong>تقييم:</strong> {item.healthy}</p>
 )}

 {/* المصادر */}
 {item.sources?.length > 0 && (
 <div className="mt-4 space-y-2">
 <h4 className="text-sm font-bold text-text-primary">مصادر لفهم أعمق:</h4>
 ...
 target="_blank"
 rel="noreferrer"
 className="flex items-center gap-2 p-3 bg-bg-surface-hover rounded-xl text-sm text-text-primary"
 >
 {src.type === 'video' && <PlayCircle size={16} className="text-[#e94826]" />}
 {['book', 'article'].includes(src.type) && <BookOpen size={16} className="text-emerald-600" />}
 {src.type === 'pod' && <Headphones size={16} className="text-emerald-600" />}
 ...
 <h3 className="text-xl font-bold text-text-primary pt-6">اختبر فهمك</h3>
 <Card className="bg-bg-surface shadow-sm">
 <CardContent className="space-y-4 p-4">
 <p className="text-base font-semibold text-text-primary">{current.q}</p>
 ...
 className={`w-full text-right ${choice === err.key ? 'border-emerald-600 bg-bg-surface-hover' : ''}`}
 >
 {err.name}
 </Button>
 ))}
